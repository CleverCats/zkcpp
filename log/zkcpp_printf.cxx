...
static u_char *zkcpp_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal, uintptr_t width);

/*----------------------------------------   暂未使用  begin   ----------------------------------*/

//该函数只不过相当于针对zkcpp_vslprintf()函数包装了一下，所以，直接研究zkcpp_vslprintf()即可
u_char *zkcpp_slprintf(u_char *buf, u_char *last, const char *fmt, ...)
{
	...
	p = zkcpp_vslprintf(buf, last, fmt, args);
	va_end(args);        //释放args   
	return p;
}

//------------------------------------------------------------------
//和上边的zkcpp_snprintf非常类似
u_char * zkcpp_snprintf(u_char *buf, size_t max, const char *fmt, ...)   //类printf()格式化函数，比较安全，max指明了缓冲区结束位置
{
	u_char   *p;
	va_list   args;

	va_start(args, fmt);
	p = zkcpp_vslprintf(buf, buf + max, fmt, args);
	va_end(args);
	return p;
}
...
u_char *zkcpp_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
	...
	int64_t    i64;   //保存%d对应的可变参 /* long long */	
	uint64_t   ui64;  //保存%ud对应的可变参,也可临时作为%f可变参的整数部分 /* unsigned long long */
	u_char     *p;    //保存%s对应的可变参  /* unsigned char*/
	double     f;     //保存%f对应的可变参
	...
	while (*fmt && buf < last)
	{
		if (*fmt == '%')
		{
			...
			//zkcpp_log_stderr(0, "数字是%010d", 12); 
			...
			case 'd': //显示整型数据，如果和u配合使用，也就是%ud,则是显示无符号整型数据
				if (sign)  //如果是有符号数
				{
					...
					//返回args原副本,args指向下一个可变参数地址(args+sizof(int))
				...
				//%i是%d的老式写法
			case 'i': //转换zkcpp_int_t型数据，如果用%ui，则转换的数据类型是zkcpp_uint_t  
				if (sign)
				{
			...
			case 'f': //一般 用于显示double类型数据，如果要显示小数部分，则要形如 %.5f  
				...
				if (frac_width) //如果是%d.2f，那么frac_width就会是这里的2
				{
					...
					{
						scale *= 10; //这可能溢出,用来乘以小数
					}

					//把小数部分取出来 ，比如如果是格式    %.2f   ，对应的参数是12.537
					// (uint64_t) ((12.537 - (double) 12) * 100 + 0.5);  
					//= (uint64_t) (0.537 * 100 + 0.5)  = (uint64_t) (53.7 + 0.5) = (uint64_t) (54.2) = 54;即12.54;+0.5可以达到四舍五入
				...
				//正整数部分，先显示出来
				buf = zkcpp_sprintf_num(buf, last, ui64, zero, 0, width); //把一个数字 比如“1234567”弄到buffer中显示
				...
				if (frac_width) //指定了显示多少位小数
				{
					...
					buf = zkcpp_sprintf_num(buf, last, frac, '0', 0, frac_width); //frac这里是小数部分，显示出来，不够的，前边填充'0'字符
				}
				fmt++;
				continue;  //重新从while开始执行
			...
			buf = zkcpp_sprintf_num(buf, last, ui64, zero, hex, width);
			fmt++;
		}
		else
		{
...
{
	//temp[21]
	u_char      *p, temp[ZKCPP_INT64_LEN + 1];   //#define ZKCPP_INT64_LEN   (sizeof("-9223372036854775808") - 1) = 20 ，注意这里是sizeof是包括末尾的\0，不是strlen；             
	size_t      len;
	uint32_t    ui32;
	...
	p = temp + ZKCPP_INT64_LEN; //ZKCPP_INT64_LEN = 20,所以 p指向的是temp[20]那个位置，也就是数组最后一个元素位置

	if (hexadecimal == 0)
	{
		if (ui64 <= (uint64_t)ZKCPP_MAX_UINT32_VALUE)   //ZKCPP_MAX_UINT32_VALUE :最大的32位无符号数：十进制是‭4294967295‬
		{
	...
	{
		//比如我显示一个1,234,567【十进制数】，他对应的16进制数实际是 12 D687 ，那怎么显示出这个12D687来呢？
		do
		{
			//0xf就是二进制的1111,位运算，ui64 & 0xf，就等于把 一个数的最末尾的4个二进制位拿出来；
	...
	//参考else if (hexadecimal == 1)，非常类似
	 ...
	 len = (temp + ZKCPP_INT64_LEN) - p;  //得到这个数字的宽度，比如 “7654321”这个数字 ,len = 7

	 while (len++ < width && buf < last)  //如果你希望显示的宽度是10个宽度【%12f】，而实际想显示的是7654321，只有7个宽度，那么这里要填充5个0进去到末尾，凑够要求的宽度
	 {
		 *buf++ = zero;  //填充zero进去到buffer中（往末尾增加），比如你用格式  
		 //zkcpp_log_stderr(0, "invalid option: %10d\n", 21); 
		 //显示的结果是：nginx: invalid option:     21---21前面有8个空格，这8个弄个，就是在这里添加进去的；
	 }

	 len = (temp + ZKCPP_INT64_LEN) - p; //还原这个len，也就是要显示的数字的实际宽度【因为上边这个while循环改变了len的值】
	 //现在还没把实际的数字比如“7654321”往buf里拷贝呢，要准备拷贝

	 //发现如果往buf里拷贝“7654321”后，会导致buf不够长【剩余的空间不够拷贝整个数字】
	 if ((buf + len) >= last)  
	 {
		 len = last - buf; //剩余的buf有多少我就拷贝多少
	 }

	 return zkcpp_cpymem(buf,  p, len); //把最新buf返回去；
}